...
デバイス初期化とRTOS起動
------------------------
main関数まで来ました。ここでシステムクロックやIO、UART(気分で入れてみた)などのデバイス類を初期化した後、FreeRTOSを起動します。
...
テストタスクの動作確認
----------------------
無事OSが起動して、各タスクが実行されるようになりました。なおタスク2でUARTにそれっぽいデータを流していますが
特に何もしていません(QEMU側でブレーク貼るとR/Wは確認可能)。
